"""LAFORET-PLUC-BE-RAP/SFM - land use conflict in restricted areas
...
list_of_new_pixels_of_land_use_conflict = list(ast.literal_eval(sys.argv[1]))
print('list_of_new_pixels_of_land_use_conflict:', list_of_new_pixels_of_land_use_conflict)
...
in_fn = os.path.join(Filepaths.folder_CONFLICT_mplc_land_use,  fn + nr_zeros * '0' + '.001')
setclone(in_fn)
amap = readmap(in_fn)
...
## SH: LPB alternation
def animate(i):
    number_of_conflict_pixels_per_year = list_of_new_pixels_of_land_use_conflict[i]
    t = i + 1
    if t < 10:
        fn = in_fn[:-3] + '00' + str(t)
    else:
    ...
    title_annotation.set_text(' - land use conflict in restricted areas probabilities classified')
