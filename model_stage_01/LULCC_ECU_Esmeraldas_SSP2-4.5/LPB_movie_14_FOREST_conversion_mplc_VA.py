"""LAFORET-PLUC-BE-RAP/SFM - Forest Conversion
...
# get the list with new converted pixels
list_of_new_pixels_of_forest_conversion = list(ast.literal_eval(sys.argv[1]))
print('list_of_new_pixels_of_forest_conversion:', list_of_new_pixels_of_forest_conversion)
...
list_all_names = {1:'= region',
                  2:'= to other land use converted forest cells per time step'
                  }
...
in_fn = os.path.join(Filepaths.folder_FOREST_PROBABILITY_MAPS_mplc_forest_conversion,  fn + nr_zeros * '0' + '.001')
setclone(in_fn)
amap = readmap(in_fn)
...
converted_pixels = axarr.text(-0.2, 0.75, '', transform=axarr.transAxes)
...
def init():
    ...
    return im, leg, converted_pixels, year, subtitle, title_annotation, title

## SH: LPB alternation
def animate(i):
    number_of_converted_pixels_per_year = list_of_new_pixels_of_forest_conversion[i]
    t = i + 1
    if t < 10:
        fn = in_fn[:-3] + '00' + str(t)
    else:
    ...
    converted_pixels.set_text('Converted pixels = ' + str(number_of_converted_pixels_per_year))
    year.set_text('Year = ' + str(init_year + t - 1))
    subtitle.set_text(country + ' ' + region + ' ' + baseline_scenario + ' ' + model_scenario + '-scenario')
    title_annotation.set_text(' - forest (disturbed+undisturbed) converted to other land use per time step')
