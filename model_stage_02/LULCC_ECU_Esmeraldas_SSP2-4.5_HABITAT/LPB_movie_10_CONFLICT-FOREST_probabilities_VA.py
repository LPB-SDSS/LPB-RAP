"""LAFORET-PLUC-BE-RAP/SFM - LPB forest land use conflict in restricted areas
...
list_of_new_pixels_of_forest_land_use_conflict = list(ast.literal_eval(sys.argv[1]))
print('list_of_new_pixels_of_forest_land_use_conflict:', list_of_new_pixels_of_forest_land_use_conflict)
...
list_all_names = {1:'1 = 0 % probability == 9',
...
in_fn = os.path.join(Filepaths.folder_CONFLICT_mplc_forest_land_use,  fn + nr_zeros * '0' + '.001')
setclone(in_fn)
amap = readmap(in_fn)
...
conflict_pixels = axarr.text(-0.2, 0.75, '', transform=axarr.transAxes)
...
def init():
    ...
    return im, leg, conflict_pixels, year, subtitle, title_annotation, title

## SH: LPB alternation
def animate(i):
    number_of_conflict_pixels_per_year = list_of_new_pixels_of_forest_land_use_conflict[i]
    t = i + 1
    if t < 10:
        fn = in_fn[:-3] + '00' + str(t)
    else:
    ...
    im = axarr.imshow(data, norm=norm_without_mv, zorder=0,\
                            cmap=cmap_long, animated=True, interpolation='nearest')
    conflict_pixels.set_text('Conflict pixels = ' + str(number_of_conflict_pixels_per_year))
    year.set_text('Year = ' + str(init_year + t - 1))
    subtitle.set_text(country + ' ' + region + ' ' + baseline_scenario + ' ' + model_scenario + '-scenario')
    title_annotation.set_text(' - forest land use conflict in restricted areas probabilities classified')
