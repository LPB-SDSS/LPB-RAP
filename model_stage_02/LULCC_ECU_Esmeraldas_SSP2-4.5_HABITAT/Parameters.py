...
LPB-RAP (SH) model stage 2 2022/3 - 2024/Q3
model stage 3 preparations 2022/11-12 (re-implementation of demand/yield in conjunction with footprint)
LPB-OC (ML) model stage 3 2022/11 - 2024/2025
LPB-SFM (DK) model stage 3 tentative in 2025
...
# 2.2) Info on simulation choices => LPB/PLUC adapter to simulate partially with demand/yield instead of footprint, habitat analysis software
...
LUT12 = 'permanent waterbodies'  # Copernicus LUT approximated
...
LUT19 = 'ocean'  # Copernicus LUT approximated

###############################################################################
# Decide if the model shall procude GIFs, this enhances the interpretation options but prolongs simulation time
def get_GIFs_production_choice():
...
def get_correction_step_list_of_immutable_LUTs():
    ...
    correction_step_list_of_immutable_LUTs = [1,5,6,7,8,9,10,11,12,13,19] # in this case the agricultural demands are only allowed to be allocated on the random agriculture area
...
def get_worst_case_scenario_decision():
    ...
    worst_case_scenario_planned = False
...
def get_number_of_time_steps():
    ...
    time_steps = 83
    ##SH: (2018 standard 83, 2025 -> 76)
    return time_steps

# TODO (model stage 2)
def get_last_simulation_year():

    last_simulation_year = 2100
    return last_simulation_year
...
def get_total_number_of_land_use_types():
    ...
    total_number_of_land_use_types = 19
...
## SH_ LPB alteration:
# TODO
def get_water_LUTs_list():
    """define here the water LUTs to extract them from the streets allocation"""

    water_LUTs_list = [12, 19]
    return water_LUTs_list
...
# SH model stage 2
# TODO
def get_connect_old_settlements_to_built_up_network_decision():
    """Decide here if in time step 1 existing unconnected settlements shall be conneted to the built up raster"""

    connect_old_settlements_to_built_up_network = True
    return connect_old_settlements_to_built_up_network

# SH model stage 2
#TODO
def get_dynamic_street_network_simulation_decision():
    """Define here if you want to connect new settlements to the existing built-up raster:"""

    connect_new_settlements_to_built_up_network = True
    return connect_new_settlements_to_built_up_network
...
# model stage 2:
# SH LPB alteration model stage 2
# TODO to simulate model stage 2
def get_habitat_analysis_choice():
    # Decide here if habiat analysis shall be perfored at all and in which module(s).
    # If set to true, you have the options to choose Ecosystem fragmentation analysis (always performed in mplc and RAP),
    # derive landscpe metrics wih PyLandStats and simulate habitat connectivity with Omniscape as well as potential habitat corrdidors with Circuitscape.
    # Choose analysis in the mplc ('mplc') landscape or the RAP landscape ('RAP') or both ('mplc+RAP')

    perform_habitat_analysis_dictionary = {
        'do_habitat_analysis': True,
        'perform_analysis_in': 'mplc+RAP'
    }
    return perform_habitat_analysis_dictionary

# SH LPB alteration model stage 2
# TODO model stage 2
def get_habitat_analysis_simulation_years_choice():
    """This method applies only to the computation time intensive external software compoenents of PyLandStats, Omniscape and Circuitscape.
    For a full chronological analysis output run the components each simulation year with 'perform_each_year':True. Otherwise note the years it shall be applied.
    For consecutive simulation of averages over time simulate each year in that time frame."""

    habitat_analysis_simulation_years_dictionary = {
        'perform_each_year': False,
        'perform_user_defined_years': [2024, 2070]
    }

    # automated prep:
    habitat_analysis_simulation_years_list = []
    if habitat_analysis_simulation_years_dictionary['perform_each_year'] == True:
        habitat_analysis_simulation_years_list = list(range(get_initial_simulation_year(), get_last_simulation_year() + 1))
    else:
        habitat_analysis_simulation_years_list = habitat_analysis_simulation_years_dictionary['perform_user_defined_years']

    return habitat_analysis_simulation_years_list

# SH LPB alteration model stage 2
# TODO to simulate model stage 2
def get_habitat_analysis_averaging_choice():
    """Define here if you want to produce averages at all"""

    perform_habitat_analysis_averaging_over_time_frames = False
    return perform_habitat_analysis_averaging_over_time_frames

# SH LPB alteration model stage 2
# TODO to simulate model stage 2
def get_habitat_analysis_averaging_time_frames_dictionary():
    """Define here the time frames."""

    habitat_analysis_averaging_time_frames_dictionary = {
        '2018–2020': {'first_year': 2018,
                      'last_year': 2020}#,
        #'2061–2080': {'first_year': 2061,
        #              'last_year': 2080}
    }
    return habitat_analysis_averaging_time_frames_dictionary

# SH: LPB alternation
# TODO to simulate model stage 2
def get_umbrella_species_ecosystem_fragmentation_analysis_mplc_RAP_analysis_choice():
    ...
    umbrella_species_ecosystem_fragmentation_analysis_mplc_RAP_analysis_choice = True
    return umbrella_species_ecosystem_fragmentation_analysis_mplc_RAP_analysis_choice

# model stage 2:
# SH: LPB alternation
# TODO to simulate model stage 2
def get_landscape_metrics_analysis_choice():
    """ if set to True, always calculats PyLandStats fractal_dimension_am. You can add additional metrics in the method below."""

    perform_landscape_metrics_analysis = True
    return perform_landscape_metrics_analysis
...
def get_PyLandStats_metrics():
    """Define here which metrics of PyLandStats you may want to additionally apply besides fractal_dimension_am
    (this is always run, if above method is set to true).
    See the provided "PyLandStats_documentation_V2.pdf in the model main folder for function names. PatchID input is fitting for analysis methods for class level.
    ATTENTION: Calculations can be quite time intensive and further metrics may need additional coding in LPB-RAP - if you do NOT want additional metrics leave the list empty: []"""

    PyLandStats_additional_metrics = [] # e.g.: 'proportion_of_landscape', 'edge_density', 'area', 'perimeter', 'perimeter_area_ratio', 'shape_index'
    return PyLandStats_additional_metrics

# model stage 2:
# SH: LPB alternation
# TODO to simulate model stage 2
def get_potential_habitat_connectivity_simulation_choice():
    ...
    habitat_connectivity_simulation_dictionary = {
        'simulate_connectivity': True,
        'simulation_in': 'mplc+RAP'
    }
    return habitat_connectivity_simulation_dictionary
...
def get_potential_habitat_corridors_simulation_choice():
    """Define here if you wish at all to simulate Potential Habitat Corridors (PHCs) for your defined regional umbrella species and if so,
     if only in the mplc ('mplc') landscape or the RAP landscape ('RAP') or both ('mplc+RAP'). Define the Circuitscape scenrio "advanced" or "pairwise".
     You will have to configure umbrella species requirements further down below.
     ATTENTION: calculation takes time for larger raster files.
     NOTE: Simulation is applied in Circuitscape and thereby fits here potential partial forest dwellers umbrella species,
     which exibit explorative behavior."""
...
def get_static_LUTs_on_which_no_allocation_occurs():  # former: get_nogo_land_use_types():
    ...
    static_LUTs_on_which_no_allocation_occurs = [12,  # waterbodies stays as is
                                                 13,  # no input stays as is
                                                 19]  # ocean stays as is
...
#######################################
# TODO for model stage 2
### 2.8) H A B I T A T - A N A L Y S I S  -  P A R A M E T E R S ##

def get_umbrella_species_name():
    umbrella_species_name = 'Panthera onca'
    return umbrella_species_name

def get_umbrella_species_altitude_range_dictionary():
    umbrella_species_altitude_range_dictionary = {
        'altitude_min': 0,
        'altitude_max': 2250
    }
    return umbrella_species_altitude_range_dictionary

def get_umbrella_species_minimum_patch_size():
    umbrella_species_minimum_patch_size = 50000000 #m2
    return umbrella_species_minimum_patch_size

def get_umbrella_species_core_habitat_LUTs_list():
    umbrella_species_core_habitat_LUTs_list = [8, 9, 22, 23, 25] # mplc + RAP LUTs
    return umbrella_species_core_habitat_LUTs_list

def get_umbrella_species_LUTs_to_resistance_values_dictionary():
    umbrella_species_LUTs_to_resistance_values_dictionary = {
        1: 95,  # built-up - anthropogenically disturbed
        2: 25,  # cropland-annual - anthropogenically disturbed
        3: 20,  # pasture - anthropogenically disturbed
        4: 15,  # agroforestry - anthropogenically disturbed - but SECONDARY FOREST TYPE WITH DISTURBANCE
        5: 22.5,  # plantation (here: oil palm) - anthropogenically disturbed
        6: 6,  # herbaceous vegetation - other habitat, no cover
        7: 4,  # shrubs - other habitat, some cover
        8: 2,  # disturbed forest - SECONDARY FOREST TYPE WITH DISTURBANCE
        9: 1,  # undisturbed forest - (SECONDARY) FOREST TYPE WITHOUT DISTURBANCE
        10: 7,  # moss, lichen, bare, sparse vegetation - other habitat, no cover
        11: 3,  # herbaceous wetland - other habitat
        12: 1.5,  # permanent waterbodies - other habitat
        13: -9999,  # no input
        14: 6,  # cropland-annual -- abandoned - other habitat
        15: 4,  # pasture -- abandoned - other habitat
        16: 2,  # agroforestry -- abandoned - SECONDARY FOREST TYPE WITHOUT DISTURBANCE
        17: 8,  # net forest -- deforested - other habitat
        18: 9,  # plantation -- harvested - other habitat
        19: -9999,  # ocean
        21: 15,  # RAP agroforestry - anthropogenically disturbed - but SECONDARY FOREST TYPE WITH DISTURBANCE
        22: 2.5,  # RAP plantation - anthropogenically disturbed - but SECONDARY FOREST TYPE WITH DISTURBANCE
        23: 1.5,  # RAP reforestation - SECONDARY FOREST TYPE WITHOUT DISTURBANCE
        24: 5,  # RAP other ecosystems - other habitat
        25: 2.5  # RAP restoration of degraded forest - anthropogenically disturbed - but SECONDARY FOREST TYPE WITH DISTURBANCE
    }
    return umbrella_species_LUTs_to_resistance_values_dictionary

def get_umbrella_species_LUTs_to_sources_in_amps_dictionary():
    umbrella_species_LUTs_to_sources_in_amps_dictionary = {
        1: 0.01,  # built-up
        2: 0.04,  # cropland-annual
        3: 0.05,  # pasture
        4: 0.07,  # agroforestry - SECONDARY FOREST TYPE WITH DISTURBANCE
        5: 0.04,  # plantation (here: oil palm)
        6: 0.17,  # herbaceous vegetation
        7: 0.25,  # shrubs
        8: 0.5,  # disturbed forest - SECONDARY FOREST TYPE WITH DISTURBANCE
        9: 1,  # undisturbed forest - (SECONDARY) FOREST TYPE WITHOUT DISTURBANCE
        10: 0.14,  # moss, lichen, bare, spare vegetation
        11: 0.03,  # herbaceous wetland
        12: 0.7,  # permanent waterbodies
        13: -9999,  # no input
        14: 0.17,  # cropland-annual -- abandoned
        15: 0.25,  # pasture -- abandoned
        16: 0.5,  # agroforestry -- abandoned - SECONDARY FOREST TYPE WITHOUT DISTURBANCE
        17: 0.13,  # net forest -- deforested
        18: 0.11,  # plantation -- harvested
        19: -9999,  # ocean
        21: 0.07,  # RAP agroforestry - SECONDARY FOREST TYPE WITH DISTURBANCE
        22: 0.4,  # RAP plantation - SECONDARY FOREST TYPE WITH DISTURBANCE
        23: 0.7,  # RAP reforestation - SECONDARY FOREST TYPE WITHOUT DISTURBANCE
        24: 0.2,  # RAP other ecosystems
        25: 0.4  # RAP restoration of degraded forest - SECONDARY FOREST TYPE WITH DISTURBANCE
    }
    return umbrella_species_LUTs_to_sources_in_amps_dictionary

def get_umbrella_species_LUTs_to_grounds_in_ohms_dictionary():
    umbrella_species_LUTs_to_grounds_in_ohms_dictionary = {
        1: 0.01,  # built-up
        2: 0.04,  # cropland-annual
        3: 0.05,  # pasture
        4: 0.07,  # agroforestry - SECONDARY FOREST TYPE WITH DISTURBANCE
    ...
    return umbrella_species_LUTs_to_grounds_in_ohms_dictionary

def get_LPBRAP_Omniscape_settings_dictionary():
    """Define here the Omniscape parameters that may be changed in LPBRAP and define if a landscape buffer >= radius is integrated"""

    LPBRAP_Omniscape_settings_dictionary = {
        'simulation_buffer_integrated': False,
        'radius': 500,  # the dispersal distance of your focal umbrella species in pixels
        'block_size': 45,  # < 10 % radius and an odd number
        'source_threshold': 0,  # Positive number. Defaults to 0. Only pixels in the source layer greater than source_threshold will be included as sources.
        'source_from_resistance': True,  # One of true, false. Should a source layer be derived using the resistance layer? If true, sources are calculated as the inverse of the resistance layer, and therefore it is not recommended that your resistance layer contain values less than 1. Sources will be set to 0 for all cells with a resistance greater than r_cutoff (described below). Defaults to false.
        'r_cutoff': 100,  # The maximum resistance value a cell can have to be included as a source. Only applies when source_from_resistance = true. Defaults to Inf (which allows all cells to be considered as sources regardless of the resistance value).
        'connect_four_neighbors_only': False,  # for computational speed you may want to set this option to True, trading in detail
        'mask_nodata': True
    }
    return LPBRAP_Omniscape_settings_dictionary

def get_Omniscape_normalized_flow_classification_dictionary():
    """Set the classification for output map of normalized cumulative flow. These classes will be derived consecutively in a for loop and the dictionary key number assigened as class.
    ATTENTION: overlapping areas SMALLER than a threshold value must be put in reverse logical order as they are allocated in a loop."""
    Omniscape_normalized_flow_interpretation_dictionary = {
        3: {'reduced_flow': {'operand': '<=',
                             'threshold': 0.7}},
        2: {'impeded_flow': {'operand': '<=',
                             'threshold': 0.47}},
        1: {'blocked_flow': {'operand': '<=',
                             'threshold': 0.27}},
        4: {'diffuse_flow': {'operand': '>',
                             'threshold': 0.7}},
        5: {'intensified_flow': {'operand': '>=',
                                 'threshold': 1.3}},
        6: {'channelized_flow': {'operand': '>',
                                 'threshold': 1.7}}
    }
    return Omniscape_normalized_flow_interpretation_dictionary

def get_LPBRAP_Circuitscape_settings_dictionary():
    """Define here Circuitscape settings"""

    LPBRAP_Circuitscape_settings_dictionary = {
        ...
        'scenario': 'pairwise' # either pairwise or advanced integrated
    }
    return LPBRAP_Circuitscape_settings_dictionary
