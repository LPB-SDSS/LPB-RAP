"""LAFORET-PLUC-BE-RAP/SFM - LUT17 deforested
...
list_of_new_deforested_area = list(ast.literal_eval(sys.argv[1]))
print('list_of_new_deforested_area:', list_of_new_deforested_area)
...
in_fn = os.path.join(Filepaths.folder_FOREST_PROBABILITY_MAPS_deforested_LUT17_probabilities_classified,  fn + nr_zeros * '0' + '.001')
setclone(in_fn)
amap = readmap(in_fn)
...
deforested_pixels = axarr.text(-0.2, 0.75, '', transform=axarr.transAxes)
...
def init():
    ...
    return im, leg, deforested_pixels, year, subtitle, title_annotation, title

## SH: LPB alternation
def animate(i):
    number_of_deforested_pixels_per_year = list_of_new_deforested_area[i]
    t = i + 1
    if t < 10:
        fn = in_fn[:-3] + '00' + str(t)
    else:
    ...
    im = axarr.imshow(data, norm=norm_without_mv, zorder=0,\
                            cmap=cmap_long, animated=True, interpolation='nearest')
    deforested_pixels.set_text('Deforested for AGB pixels = ' + str(number_of_deforested_pixels_per_year))
    year.set_text('Year = ' + str(init_year + t - 1))
    subtitle.set_text(country + ' ' + region + ' ' + baseline_scenario + ' ' + model_scenario + '-scenario')
    title_annotation.set_text(' - deforestation by outtake of input biomass only')
