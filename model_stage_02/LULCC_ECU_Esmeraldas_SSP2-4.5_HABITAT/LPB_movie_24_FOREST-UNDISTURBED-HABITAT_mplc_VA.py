"""LAFORET-PLUC-BE-RAP/SFM - LPB mplc forest remaining truly undisturbed habitat (no impact, no fringe effects)
runs different for BAU and worst case scenario (BAU with region and restricted areas background)
...
list_all_names = {1:'= gross undisturbed forest habitat',
                  2:'= net undisturbed forest habitat',
                  3:'= undisturbed forest habitat in restricted areas',
                  8:'= region',
                  9:'= restricted areas'}
...
in_fn = os.path.join(Filepaths.folder_FOREST_PROBABILITY_MAPS_mplc_forest_undisturbed_habitat,  fn + nr_zeros * '0' + '.001')
setclone(in_fn)
amap = readmap(in_fn)
...
def animate(i):
    ...
    title_annotation.set_text(' - remaining nominal undisturbed forest habitat')
