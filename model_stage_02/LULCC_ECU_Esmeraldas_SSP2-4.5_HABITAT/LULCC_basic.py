...
class LandUseType:
    ...
    def _get_dynamic_streets_suitability(self, distances_to_dynamic_streets_map):
        """ JV: Return suitability map based on distance to streets."""
        variables_list = self.variables_dictionary.get(2)
        direction = variables_list[0]
        maximum_distance = variables_list[1]
        if self.stochastic_distance == 1:
            maximum_distance = 2 * celllength() + mapuniform() * \
                               (2 * maximum_distance - 2 * celllength())
            print('maximum distance dynamic streets is:', int(maximum_distance))
        friction = variables_list[2]
        relation_type = variables_list[3]

        # JV: Influence up to some maximum distance
        cut_off_map = ifthen(distances_to_dynamic_streets_map <
                             maximum_distance, distances_to_dynamic_streets_map * direction)
        normalized_map = self._normalize_map(cut_off_map)
        # JV: Implement linear (0), exponential (1) or inv. proportional (2) relation
        if relation_type == 0:
            relation_map = normalized_map
        ...
        dynamic_street_suitability_map = cover(relation_map, scalar(self.null_mask_map))  # SH: changed null mask_map to scalar
        print('dynamic street-suitability for LUT', self.type_number, 'calculated')
        return dynamic_street_suitability_map
...
# SH placeholder for SFM model stage 3
    ...
    def create_initial_suitability_map(self, distances_to_streets_map, distances_to_freshwater_map, distances_to_cities_map):
        ...
        for a_factor in self.suitability_factors_list:
            if a_factor == 2:
                if (Parameters.get_connect_old_settlements_to_built_up_network_decision() == False and Parameters.get_dynamic_street_network_simulation_decision() == False):
                ...
                if (Parameters.get_connect_old_settlements_to_built_up_network_decision() == True or Parameters.get_dynamic_street_network_simulation_decision() == True):
                    # street suitability - static - stays 2
                    self.initial_suitability_map += self.weights_list[iteration] * \
                                                    self._get_distance_street_suitability(distances_to_streets_map)
                    self.weight_initial_suitability_map += self.weights_list[iteration]
                    print('static distance to streets suitability for LUT', self.type_number, 'calculated')
                    self.static_streets_suitability_map = spatial(scalar(0))
                    self.static_streets_suitability_map += self.weights_list[iteration] * \
                                                    self._get_distance_street_suitability(distances_to_streets_map)
                    print('calculated initial static streets suitability to be subtracted after time step 1 - application of dyanamic streets suitabilty')
            elif a_factor == 3:
    ...
    # SH: LPB alternation
    def get_total_suitability_map(self, environment_map, distances_to_settlements_map, population_map, net_forest_map, distances_to_dynamic_streets_map):
            ...
            if a_factor == 2:
                if Parameters.get_connect_old_settlements_to_built_up_network_decision() == True or Parameters.get_dynamic_street_network_simulation_decision() == True:
                    suitability_map += self.weights_list[iteration] * \
                                       self._get_dynamic_streets_suitability(distances_to_dynamic_streets_map=distances_to_dynamic_streets_map)
                    print('dynamic streets suitability for LUT', self.type_number, 'calculated')
                else: pass
            elif a_factor in (3,4): pass
                # JV: Static factors already captured in the initial suitability map
                # SH: static factors are now only:
                # 2 = street distance -> user-defined dynamic
        ...
        if self.time_step > 1:
            ...
            if (Parameters.get_connect_old_settlements_to_built_up_network_decision() == True or Parameters.get_dynamic_street_network_simulation_decision() == True):
                suitability_map += self.initial_suitability_map
                suitability_map -= self.static_streets_suitability_map # employs the dynamic streets suitability map
...
class LandUse:
    ...
    def update_settlements(self):
        ...
        print('\ncalculate new settlements and if user-defined also streets')

        # insertion model stage 2: get water LUTs and build slope map without these areas
        water_LUTs_list = Parameters.get_water_LUTs_list()
        water_map = boolean(self.null_mask_map)
        for a_water_LUT in water_LUTs_list:
            temp_map = ifthen(scalar(self.environment_map) == scalar(a_water_LUT),
                              boolean(1))
            water_map = cover(boolean(temp_map), boolean(water_map))

        self.slope_without_water_map = ifthen(scalar(water_map) != scalar(1),
                                              scalar(self.slope_map))
        ...
        if self.time_step == 1:
            ...
            if Parameters.get_connect_old_settlements_to_built_up_network_decision() == True:
                # find all singular singular settlements in the current environment map
                built_up_map = ifthen(scalar(self.environment_map) == scalar(1),
                                      scalar(1))


                window_length = 3 * Parameters.get_cell_length_in_m()
                number_neighbors_map = windowtotal(built_up_map, window_length) - built_up_map
                singular_settlements_map = ifthen(scalar(number_neighbors_map) == scalar(0),
                                           # determined by missing pixels in the window (less than 9 (-1) present)
                                           scalar(1))
                singular_settlements = int(maptotal(scalar(singular_settlements_map)))

                # prepare maps as input
                built_up_map = cover(scalar(built_up_map), scalar(self.null_mask_map))
                singular_settlements_map = cover(scalar(singular_settlements_map), scalar(self.null_mask_map))

                # create new path to existing built-up
                print('calculate spread map distances over slope to existing', singular_settlements, 'singular settlements:')
                setglobaloption('unitcell')
                built_up_spread_map = spread(boolean(built_up_map), 0, self.slope_without_water_map)
                print('create ldd:')
                setglobaloption('lddin')
                ldd = lddcreate(built_up_spread_map, 999999, 999999, 999999, 999999)
                print('create new paths aka streets')
                new_street_pixels_map = path(ldd, boolean(singular_settlements_map))
                # eliminate zeros
                pure_new_street_pixels_map = ifthen(scalar(new_street_pixels_map) == scalar(1),
                                                    scalar(1))
                setglobaloption('unittrue')

                # update the landscape
                self.environment_map = cover(nominal(pure_new_street_pixels_map), nominal(self.environment_map))

                # update the streets map
                self.streets_map = cover(boolean(pure_new_street_pixels_map), boolean(self.streets_map))
            ...
            if self.population > self.population_last_time_step:
                ...
                if only_one_new_settlement_per_year is True:
                    ...
                    if Parameters.get_dynamic_street_network_simulation_decision() == True:
                        # find all singular singular settlements in the current environment map
                        built_up_map = ifthen(scalar(self.environment_map) == scalar(1),
                                              scalar(1))
                        ...
                        # prepare maps as input
                        built_up_map = cover(scalar(built_up_map), scalar(self.null_mask_map))
                        new_settlement_map = cover(scalar(new_settlement_map), scalar(self.null_mask_map))

                        # create new path to existing built-up
                        print('calculate spread map distances slope to the one new settlement')
                        setglobaloption('unitcell')
                        built_up_spread_map = spread(boolean(built_up_map), 0, self.slope_without_water_map)
                        print('create ldd:')
                        setglobaloption('lddin')
                        ldd = lddcreate(built_up_spread_map, 999999, 999999, 999999, 999999)
                        print('create new path aka street:')
                        new_street_pixels_map = path(ldd, boolean(new_settlement_map))
                        # eliminate zeros
                        pure_new_street_pixels_map = ifthen(scalar(new_street_pixels_map) == scalar(1),
                                                            scalar(1))
                        setglobaloption('unittrue')
                ...
                # OPTION B) GET A PROPORTIONAL GROWTH IN THE NUMBER OF SETTLEMENTS
                    ...
                    if Parameters.get_dynamic_street_network_simulation_decision() == True:
                        ...
                        # prepare maps as input
                        built_up_map = cover(scalar(built_up_map), scalar(self.null_mask_map))
                        new_settlements_map = cover(scalar(new_settlements_map), scalar(self.null_mask_map))
                        new_settlements = int(maptotal(scalar(new_settlements_map)))

                        # create new path to existing built-up
                        print('calculate spread map distances over slope to', new_settlements, 'new settlements')
                        setglobaloption('unitcell')
                        built_up_spread_map = spread(boolean(built_up_map), 0, self.slope_without_water_map)
                        print('create ldd:')
                        setglobaloption('lddin')
                        ldd = lddcreate(built_up_spread_map, 999999, 999999, 999999, 999999)
                        print('create new paths aka streets:')
                        new_street_pixels_map = path(ldd, boolean(new_settlements_map))
                        # eliminate zeros
                        pure_new_street_pixels_map = ifthen(scalar(new_street_pixels_map) == scalar(1),
                                                            scalar(1))
                        setglobaloption('unittrue')
        ...
        self.distances_to_dynamic_streets_map = spread(boolean(self.streets_map), 0, 1)
    ...
    def update_anthropogenic_impact_buffer(self):
        ...
        anthropogenic_landscape_features_map = ifthen(scalar(self.environment_map) == scalar(1),
                                                      boolean(1))
        anthropogenic_landscape_features_map = cover(boolean(anthropogenic_landscape_features_map), boolean(self.null_mask_map))
        ...
        # further information
        print('\nwith a buffer of', Parameters.get_anthropogenic_impact_distance_in_m(), 'meter to anthropogenic landscape features, '
        ...
        if Parameters.get_connect_old_settlements_to_built_up_network_decision() == False and Parameters.get_dynamic_street_network_simulation_decision() == False:
            for a_type in self.land_use_types:
                suitability_map = a_type.get_total_suitability_map(environment_map=self.environment_map,
                                                                   ...
                                                                   distances_to_dynamic_streets_map=spatial(0))
        ...
        if Parameters.get_connect_old_settlements_to_built_up_network_decision() == True or Parameters.get_dynamic_street_network_simulation_decision() == True:
            for a_type in self.land_use_types:
                suitability_map = a_type.get_total_suitability_map(environment_map=self.environment_map,
                                                                   ...
                                                                   population_map=self.population_map,
                                                                   net_forest_map=self.net_forest_map,
                                                                   distances_to_dynamic_streets_map=self.distances_to_dynamic_streets_map)
                suitability_maps.append(suitability_map)
    ...
    def correct_AGB_map_after_allocation(self):
        ...
        print('\ncorrecting AGB map after allocation')
    ...
    def update_built_up_area_last_time_step(self):
        ...
        print('\nupdating built-up area last time step done')
    ...
    def update_plantation_information(self):
        ...
        print('\nupdating information from new or deforested plantations initialized ...')
    ...
    def update_succession(self):
        ...
        print('\ncalculating succession initialized ...')
    ...
    def update_AGB_map_new_agroforestry_and_disturbed_pixels(self):
        ...
        print('\nupdating AGB new agroforestry and forest')
    ...
    def update_forest_fringe_disturbed(self):
        ...
        print('\nupdating disturbed forest fringe')
    ...
    def update_forest_degradation_and_regeneration(self):
        ...
        print('\nupdating forest degradation and regeneration initiated ...')
    ...
    def get_environment(self):
        ...
        print('\ncurrent environment map returned')
