...
# get the habitat analysis dictionary
...
file_initial_streets_input = os.path.join(
    folder_inputs_initial, 'initial_streets_input')
...
if habitat_analysis_dictionary['do_habitat_analysis'] == True:
    if habitat_connectivity_simulation_dictionary['simulate_connectivity'] == True:
        file_initial_habitat_analysis_Omniscape_configuration_ini = os.path.join(
            folder_inputs_initial, 'initial_habitat_analysis_Omniscape_configuration.ini')
    if PHC_simulation_dictionary['simulate_PHCs'] == True:
        file_initial_habitat_analysis_Circuitscape_configuration_ini = os.path.join(
            folder_inputs_initial, 'initial_habitat_analysis_Circuitscape_configuration.ini')
...
# for model stage 2:
file_static_excluded_areas_for_umbrella_species_input = os.path.join(
    folder_inputs_static, 'static_excluded_areas_for_umbrella_species_input')
...
# model_stage_2
if habitat_analysis_dictionary['perform_analysis_in'] == 'mplc' or habitat_analysis_dictionary['perform_analysis_in'] == 'mplc+RAP':
    folder_HABITAT_ANALYSIS_MPLC = os.path.join(
        os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC')
    os.makedirs(folder_HABITAT_ANALYSIS_MPLC, exist_ok=True)

    folder_altitude_range_habitat_environment_map_mplc = os.path.join(
        os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'altitude_range_habitat_environment_map')
    os.makedirs(folder_altitude_range_habitat_environment_map_mplc, exist_ok=True)

    if Parameters.get_umbrella_species_ecosystem_fragmentation_analysis_mplc_RAP_analysis_choice() == True:
        folder_ecosystem_fragmentation_mplc = os.path.join(
            os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'ecosystem_fragmentation')
        os.makedirs(folder_ecosystem_fragmentation_mplc, exist_ok=True)

        folder_ecosystem_fragmentation_PatchID_mplc = os.path.join(
            os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'ecosystem_fragmentation_PatchID')
        os.makedirs(folder_ecosystem_fragmentation_PatchID_mplc, exist_ok=True)

    if Parameters.get_landscape_metrics_analysis_choice() == True:
        folder_landscape_metrics_mplc = os.path.join(
            os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'landscape_metrics')
        os.makedirs(folder_landscape_metrics_mplc, exist_ok=True)
        ...
        if habitat_connectivity_simulation_dictionary['simulation_in'] == 'mplc' or habitat_connectivity_simulation_dictionary['simulation_in'] == 'mplc+RAP'\
                or PHC_simulation_dictionary['simulation_in'] == 'mplc' or PHC_simulation_dictionary['simulation_in'] == 'mplc+RAP':
            folder_habitat_analysis_mplc_inputs = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS')
            os.makedirs(folder_habitat_analysis_mplc_inputs, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_Omniscape_ini_files = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'Omniscape_ini_files')
            os.makedirs(folder_habitat_analysis_mplc_inputs_Omniscape_ini_files, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_Circuitscape_ini_files = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'Circuitscape_ini_files')
            os.makedirs(folder_habitat_analysis_mplc_inputs_Circuitscape_ini_files, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_resistance_map = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'resistance_files_map')
            os.makedirs(folder_habitat_analysis_mplc_inputs_resistance_map, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_resistance_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'resistance_files_ascii')
            os.makedirs(folder_habitat_analysis_mplc_inputs_resistance_ascii, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_sources_map = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'sources_files_map')
            os.makedirs(folder_habitat_analysis_mplc_inputs_sources_map, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_sources_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'sources_files_ascii')
            os.makedirs(folder_habitat_analysis_mplc_inputs_sources_ascii, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_grounds_map = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'grounds_files_map')
            os.makedirs(folder_habitat_analysis_mplc_inputs_grounds_map, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_grounds_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'grounds_files_ascii')
            os.makedirs(folder_habitat_analysis_mplc_inputs_grounds_ascii, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_habitat_map = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'habitat_files_map')
            os.makedirs(folder_habitat_analysis_mplc_inputs_habitat_map, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_habitat_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'habitat_files_ascii')
            os.makedirs(folder_habitat_analysis_mplc_inputs_habitat_ascii, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_polygons_map = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'polygons_files_map')
            os.makedirs(folder_habitat_analysis_mplc_inputs_polygons_map, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_polygons_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'polygons_files_ascii')
            os.makedirs(folder_habitat_analysis_mplc_inputs_polygons_ascii, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_points_map = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'points_files_map')
            os.makedirs(folder_habitat_analysis_mplc_inputs_points_map, exist_ok=True)

            folder_habitat_analysis_mplc_inputs_points_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'INPUTS', 'points_files_ascii')
            os.makedirs(folder_habitat_analysis_mplc_inputs_points_ascii, exist_ok=True)

            folder_habitat_analysis_mplc_outputs = os.path.join(
                os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS')
            os.makedirs(folder_habitat_analysis_mplc_outputs, exist_ok=True)

            if habitat_connectivity_simulation_dictionary['simulate_connectivity'] == True:
                folder_habitat_analysis_mplc_outputs_Omniscape = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Omniscape')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Omniscape, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Omniscape_PCRaster = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Omniscape_to_PCRaster')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Omniscape_PCRaster, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Omniscape_PCRaster_normalized = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Omniscape_to_PCRaster', 'normalized_cumulative_current')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Omniscape_PCRaster_normalized, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Omniscape_PCRaster_cumulative = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Omniscape_to_PCRaster', 'cumulative_current')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Omniscape_PCRaster_cumulative, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Omniscape_PCRaster_flow = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Omniscape_to_PCRaster', 'flow_potential')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Omniscape_PCRaster_flow, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Omniscape_transformed = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Omniscape_transformed')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Omniscape_transformed, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Omniscape_transformed_normalized_reclassified = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Omniscape_transformed', 'normalized_reclassified')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Omniscape_transformed_normalized_reclassified, exist_ok=True)

            if PHC_simulation_dictionary['simulate_PHCs'] == True:
                folder_habitat_analysis_mplc_outputs_Circuitscape = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Circuitscape')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Circuitscape, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Circuitscape_to_PCRaster = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Circuitscape_to_PCRaster')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Circuitscape_to_PCRaster, exist_ok=True)

                if PHC_simulation_dictionary['simulation_scenario'] == 'advanced':
                    folder_habitat_analysis_mplc_outputs_Circuitscape_to_PCRaster_curmap = os.path.join(
                        os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Circuitscape_to_PCRaster', 'curmap')
                    os.makedirs(folder_habitat_analysis_mplc_outputs_Circuitscape_to_PCRaster_curmap, exist_ok=True)

                    folder_habitat_analysis_mplc_outputs_Circuitscape_to_PCRaster_voltmap = os.path.join(
                        os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Circuitscape_to_PCRaster', 'voltmap')
                    os.makedirs(folder_habitat_analysis_mplc_outputs_Circuitscape_to_PCRaster_voltmap, exist_ok=True)

                if PHC_simulation_dictionary['simulation_scenario'] == 'pairwise':
                    folder_habitat_analysis_mplc_outputs_Circuitscape_to_PCRaster_cum_curmap = os.path.join(
                        os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Circuitscape_to_PCRaster', 'cum_curmap')
                    os.makedirs(folder_habitat_analysis_mplc_outputs_Circuitscape_to_PCRaster_cum_curmap, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Circuitscape_transformed = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Circuitscape_transformed')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Circuitscape_transformed, exist_ok=True)

                folder_habitat_analysis_mplc_outputs_Circuitscape_transformed_voltmap_rescaled = os.path.join(
                    os.getcwd(), folder_outputs, 'LPB', 'HABITAT_ANALYSIS_MPLC', 'OUTPUTS', 'Circuitscape_transformed', 'voltmap_rescaled')
                os.makedirs(folder_habitat_analysis_mplc_outputs_Circuitscape_transformed_voltmap_rescaled, exist_ok=True)

if habitat_analysis_dictionary['perform_analysis_in'] == 'RAP' or habitat_analysis_dictionary['perform_analysis_in'] == 'mplc+RAP':
    folder_HABITAT_ANALYSIS_RAP = os.path.join(
        os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP')
    os.makedirs(folder_HABITAT_ANALYSIS_RAP, exist_ok=True)

    folder_altitude_range_habitat_environment_map_RAP = os.path.join(
        os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'altitude_range_habitat_environment_map')
    os.makedirs(folder_altitude_range_habitat_environment_map_RAP, exist_ok=True)

    if Parameters.get_umbrella_species_ecosystem_fragmentation_analysis_mplc_RAP_analysis_choice() == True:
        folder_ecosystem_fragmentation_RAP = os.path.join(
            os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'ecosystem_fragmentation')
        os.makedirs(folder_ecosystem_fragmentation_RAP, exist_ok=True)

        folder_ecosystem_fragmentation_PatchID_RAP = os.path.join(
            os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'ecosystem_fragmentation_PatchID')
        os.makedirs(folder_ecosystem_fragmentation_PatchID_RAP, exist_ok=True)

    if Parameters.get_landscape_metrics_analysis_choice() == True:
        folder_landscape_metrics_RAP = os.path.join(
            os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'landscape_metrics')
        os.makedirs(folder_landscape_metrics_RAP, exist_ok=True)

        if habitat_connectivity_simulation_dictionary['simulation_in'] == 'RAP' or habitat_connectivity_simulation_dictionary['simulation_in'] == 'mplc+RAP'\
                or PHC_simulation_dictionary['simulation_in'] == 'RAP' or PHC_simulation_dictionary['simulation_in'] == 'mplc+RAP':
            folder_habitat_analysis_RAP_inputs = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS')
            os.makedirs(folder_habitat_analysis_RAP_inputs, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_Omniscape_ini_files = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'Omniscape_ini_files')
            os.makedirs(folder_habitat_analysis_RAP_inputs_Omniscape_ini_files, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_Circuitscape_ini_files = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'Circuitscape_ini_files')
            os.makedirs(folder_habitat_analysis_RAP_inputs_Circuitscape_ini_files, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_resistance_map = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'resistance_files_map')
            os.makedirs(folder_habitat_analysis_RAP_inputs_resistance_map, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_resistance_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'resistance_files_ascii')
            os.makedirs(folder_habitat_analysis_RAP_inputs_resistance_ascii, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_sources_map = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'sources_files_map')
            os.makedirs(folder_habitat_analysis_RAP_inputs_sources_map, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_sources_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'sources_files_ascii')
            os.makedirs(folder_habitat_analysis_RAP_inputs_sources_ascii, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_grounds_map = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'grounds_files_map')
            os.makedirs(folder_habitat_analysis_RAP_inputs_grounds_map, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_grounds_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'grounds_files_ascii')
            os.makedirs(folder_habitat_analysis_RAP_inputs_grounds_ascii, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_habitat_map = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'habitat_files_map')
            os.makedirs(folder_habitat_analysis_RAP_inputs_habitat_map, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_habitat_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'habitat_files_ascii')
            os.makedirs(folder_habitat_analysis_RAP_inputs_habitat_ascii, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_polygons_map = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'polygons_files_map')
            os.makedirs(folder_habitat_analysis_RAP_inputs_polygons_map, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_polygons_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'polygons_files_ascii')
            os.makedirs(folder_habitat_analysis_RAP_inputs_polygons_ascii, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_points_map = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'points_files_map')
            os.makedirs(folder_habitat_analysis_RAP_inputs_points_map, exist_ok=True)

            folder_habitat_analysis_RAP_inputs_points_ascii = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'INPUTS', 'points_files_ascii')
            os.makedirs(folder_habitat_analysis_RAP_inputs_points_ascii, exist_ok=True)

            folder_habitat_analysis_RAP_outputs = os.path.join(
                os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS')
            os.makedirs(folder_habitat_analysis_RAP_outputs, exist_ok=True)

            if habitat_connectivity_simulation_dictionary['simulate_connectivity'] == True:
                folder_habitat_analysis_RAP_outputs_Omniscape = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Omniscape')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Omniscape, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Omniscape_PCRaster = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Omniscape_to_PCRaster')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Omniscape_PCRaster, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Omniscape_PCRaster_normalized = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Omniscape_to_PCRaster',
                    'normalized_cumulative_current')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Omniscape_PCRaster_normalized, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Omniscape_PCRaster_cumulative = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Omniscape_to_PCRaster',
                    'cumulative_current')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Omniscape_PCRaster_cumulative, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Omniscape_PCRaster_flow = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Omniscape_to_PCRaster',
                    'flow_potential')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Omniscape_PCRaster_flow, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Omniscape_transformed = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Omniscape_transformed')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Omniscape_transformed, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Omniscape_transformed_normalized_reclassified = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Omniscape_transformed',
                    'normalized_reclassified')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Omniscape_transformed_normalized_reclassified,
                            exist_ok=True)

            if PHC_simulation_dictionary['simulate_PHCs'] == True:
                folder_habitat_analysis_RAP_outputs_Circuitscape = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Circuitscape')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Circuitscape, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Circuitscape_to_PCRaster = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Circuitscape_to_PCRaster')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Circuitscape_to_PCRaster, exist_ok=True)

                if PHC_simulation_dictionary['simulation_scenario'] == 'advanced':
                    folder_habitat_analysis_RAP_outputs_Circuitscape_to_PCRaster_curmap = os.path.join(
                        os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Circuitscape_to_PCRaster',
                        'curmap')
                    os.makedirs(folder_habitat_analysis_RAP_outputs_Circuitscape_to_PCRaster_curmap, exist_ok=True)

                    folder_habitat_analysis_RAP_outputs_Circuitscape_to_PCRaster_voltmap = os.path.join(
                        os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Circuitscape_to_PCRaster',
                        'voltmap')
                    os.makedirs(folder_habitat_analysis_RAP_outputs_Circuitscape_to_PCRaster_voltmap, exist_ok=True)

                if PHC_simulation_dictionary['simulation_scenario'] == 'pairwise':
                    folder_habitat_analysis_RAP_outputs_Circuitscape_to_PCRaster_cum_curmap = os.path.join(
                        os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Circuitscape_to_PCRaster',
                        'cum_curmap')
                    os.makedirs(folder_habitat_analysis_RAP_outputs_Circuitscape_to_PCRaster_cum_curmap, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Circuitscape_transformed = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Circuitscape_transformed')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Circuitscape_transformed, exist_ok=True)

                folder_habitat_analysis_RAP_outputs_Circuitscape_transformed_voltmap_rescaled = os.path.join(
                    os.getcwd(), folder_outputs, 'RAP', 'HABITAT_ANALYSIS_RAP', 'OUTPUTS', 'Circuitscape_transformed',
                    'voltmap_rescaled')
                os.makedirs(folder_habitat_analysis_RAP_outputs_Circuitscape_transformed_voltmap_rescaled,
                            exist_ok=True)
