"""LAFORET-PLUC-BE-RAP/SFM - Input map visualization initial LULC
...
wd = os.getcwd()
in_fn = os.path.join(wd, Filepaths.folder_inputs_initial, fn)
setclone(in_fn)
amap = readmap(in_fn)
...
def animate(i):
    ...
    title_annotation.set_text(' - initial Copernicus based hybrid map before correction step')
    title.set_text('LPB LULCC SIMULATION - before correction step')
    return im, leg, year, subtitle, title_annotation, title
