...
# Model stage 2 (MS2) authors: SH, 2024/Q3
...
years_for_plotting = list(
  'weak_conservation' = ymd(paste(c(2018, 2024, 2060, 2070, 2100), 12, 31, sep = '-')),
  'enforced_conservation' = ymd(paste(c(2018, 2024, 2060, 2070, 2100), 12, 31, sep = '-')),
  'no_conservation' = ymd(paste(c(2018, 2024, 2060, 2070, 2100), 12, 31, sep = '-')),
  'combined' = ymd(paste(c(2018, 2024, 2060, 2070, 2100), 12, 31, sep = '-')),
...
ms2_fragmentation_simulated <- TRUE

# MS2: if you simulated potential habitat corridors set the variable to true
ms2_potential_habitat_corridors_simulated <- TRUE
...
### MS2: fragmentation mplc RAP juxtaposition diagram #####################
# Status => WIP
if (model_stage == 'MS2' & ms2_fragmentation_simulated == TRUE) {
  plot_title <- 'Ecosystem fragmentation mplc RAP juxtaposition no FLR vs. potential FLR (user-defined)'
  # code entire diagram in brackets and for scenario use
  
  legend_mplc_title <- "mplc ecosystem fragmentation - no FLR scenario"
  legend_RAP_title <- "RAP ecosystem fragmentation - potential FLR scenario"
  x_axis_title <- "Year"
  ...
  pl <- pl + geom_line(data = mplc_total_area, aes(x = year_date, y = value, color = "Total area of patches in 1000 km\u00b2", linetype = "Total area of patches in 1000 km\u00b2"))
  ...
  pl <- pl + scale_color_manual(values = c("#3b528b", "#3b528b"), name = legend_mplc_title, guide = guide_legend(order = 1)) # blue lines 
  pl <- pl + scale_linetype_manual(values = c("Number of patches" = 'solid', "Total area of patches in 1000 km\u00b2" = 'longdash'), 
                                   name = legend_mplc_title,
                                   guide = guide_legend(order = 1))
  #pl <- pl + scale_linetype_discrete(guide = guide_legend(order = 1))
  # plot the RAP values
  ...
  pl <- pl + geom_line(data = RAP_total_area, aes(x = year_date, y = value, color = "Total area of patches in 1000 km\u00b2", linetype = "Total area of patches in 1000 km\u00b2"))
  ...
  pl <- pl + scale_color_manual(values = c("#21918c", "#21918c"), name = legend_RAP_title, guide = guide_legend(order = 2)) # petrol lines
  pl <- pl + scale_linetype_manual(values = c("Number of patches" = 'solid', "Total area of patches in 1000 km\u00b2" = 'longdash'), 
                                   name = legend_RAP_title,
                                   guide = guide_legend(order = 2))
  #pl <- pl + scale_linetype_discrete(guide = guide_legend(order = 2))
  # apply scenarios
  pl <- pl + facet_wrap(~scenario_name_output, scales = 'free_x')
  ...
  #pl <- pl + scale_linetype_discrete(guide = guide_legend(order = 3))
  # subset_only_for_probing_dates <- . %>%
  #   filter(year_date %in% years_for_plotting[[first(.$scenario)]])
  # pl <- pl + geom_point(data = subset_only_for_probing_dates)
  pl <- pl + theme_LPB_RAP 
...
### MS2: reclassified Omniscape normalized diagram #####
mplc_Omniscape_analysis_performed <- TRUE
RAP_Omniscape_analysis_performed <- TRUE
# > mplc ####
if (mplc_Omniscape_analysis_performed == TRUE) {
  plot_title <- "Habitat connectivity normalized cumulative current reclassified mplc"
  legend_title <- "Flow classification"
  x_axis_title <- "Year"
  y_axis_title <- paste("Pixels [1 pixel = 1 ", landscape_area_unit, ']')
  
  sorted_data <- input_data$LPB_Omniscape_transformed_normalized_pixels[[1]] %>%
    mutate(Aspect = fct_inorder(Aspect))
  
  pl <- ggplot(sorted_data, aes(x = year_date, y = value, color = Aspect))
  pl <- pl + geom_line()
  pl <- pl + scale_color_viridis_d(option = user_defined_color_spectrum)
  pl <- pl + geom_vline(aes(xintercept = years_for_plotting[['population_peak']],
                            linetype = "Population peak year"),
...
# > RAP ####
if (RAP_Omniscape_analysis_performed == TRUE) {
  plot_title <- "Habitat connectivity normalized cumulative current reclassified RAP"
  legend_title <- "Flow classification"
  x_axis_title <- "Year"
  y_axis_title <- paste("Pixels [1 pixel = 1 ", landscape_area_unit, ']')
  
  sorted_data <- input_data$RAP_Omniscape_transformed_normalized_pixels[[1]] %>%
    mutate(Aspect = fct_inorder(Aspect))
